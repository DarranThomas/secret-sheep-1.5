...
buffer *mb;
	...
	for (j = 0; j < p->conf.parsed_query->used; j++) {
		...
		case FORMAT_REMOTE_USER:
			if (NULL != (remote_user = (data_string *)array_get_element(con->environment, CONST_STR_LEN("REMOTE_USER"))) && remote_user->value->used > 1) {
			} else {
...
*/
//log_error_write(srv, __FILE__, __LINE__, "ss", "checkpoint ", "01");

 mb = buffer_init();
buffer_prepare_append(mb, 2048);
//log_error_write(srv, __FILE__, __LINE__, "ss", "checkpoint ", "1");
                                buffer_append_string(mb, get_http_method_name(con->request.http_method));
                                buffer_append_string_len(mb, CONST_STR_LEN(" "));
                                buffer_append_string_buffer(mb, con->request.orig_uri);
                                buffer_append_string_len(mb, CONST_STR_LEN(" "));
                                buffer_append_string(mb, get_http_version_name(con->request.http_version));
//log_error_write(srv, __FILE__, __LINE__, "ss", "checkpoint ", "2");
b[j].buffer_type = MYSQL_TYPE_STRING;
b[j].buffer = mb->ptr;
b[j].buffer_length = 2048;
b[j].length = &mb->used;
//log_error_write(srv, __FILE__, __LINE__, "ss", "checkpoint ", "3");
		...
		case FORMAT_HEADER:
			b[j].buffer_type = MYSQL_TYPE_STRING;
			b[j].buffer_length = 2048;
			if (NULL != p->conf.parsed_query->ptr[j]->string && NULL != (request_header = (data_string *) array_get_element(con->request.headers, CONST_STR_LEN(p->conf.parsed_query->ptr[j]->string->ptr)))) {
		...
		case FORMAT_RESPONSE_HEADER:
			b[j].buffer_type = MYSQL_TYPE_STRING;
			b[j].buffer_length = 2048;
			if (NULL != p->conf.parsed_query->ptr[j]->string && NULL != (response_header = (data_string *) array_get_element(con->response.headers, CONST_STR_LEN(p->conf.parsed_query->ptr[j]->string->ptr)))) {
		...
		case FORMAT_ENV:
			b[j].buffer_type = MYSQL_TYPE_STRING;
			b[j].buffer_length = 2048;
			if (NULL != p->conf.parsed_query->ptr[j]->string && NULL != (environment = (data_string *) array_get_element(con->environment, CONST_STR_LEN(p->conf.parsed_query->ptr[j]->string->ptr)))) {
...
int mod_mysql_accesslog_plugin_init(plugin *p)
{
	...
	p->handle_response_done = mod_mysql_accesslog_write;
